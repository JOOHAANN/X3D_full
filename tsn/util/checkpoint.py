...
class CheckPointer:
    ...
    def load(self, f=None, use_latest=True, map_location=None):
        ...
        if isinstance(checkpoint, dict) and "model" in checkpoint:
            model_state = checkpoint.pop("model")
        elif isinstance(checkpoint, dict) and "model_state" in checkpoint:
            model_state = checkpoint.pop("model_state")
        ...
        for name, tensor in model_state.items():
            if name not in current_state:
                skipped.append(name)
                continue
            if hasattr(tensor, "shape") and current_state[name].shape != tensor.shape:
                skipped.append(name)
                continue
        ...
        if skipped:
            ...
            suffix = "" if len(skipped) <= 8 else f", ... (+{len(skipped) - 8})"
            self.logger.info(f"Skipped {len(skipped)} incompatible checkpoint tensors: {preview}{suffix}")
        if missing:
            preview = ", ".join(missing[:8])
            suffix = "" if len(missing) <= 8 else f", ... (+{len(missing) - 8})"
            self.logger.info(f"Missing {len(missing)} model tensors after checkpoint load: {preview}{suffix}")
        if unexpected:
            preview = ", ".join(unexpected[:8])
            suffix = "" if len(unexpected) <= 8 else f", ... (+{len(unexpected) - 8})"
            self.logger.info(f"Unexpected {len(unexpected)} checkpoint tensors: {preview}{suffix}")
